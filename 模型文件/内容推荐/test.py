...
def run_model(json_text):
    ...
    if json_data.get('studyTime') is not None and json_data.get('totalTime') is not None:
    ...
    df['interest'] = df['interest'].apply(lambda x: x[1:-1].split('，'))
    ...
    field_dims = np.array(
        [1274, 7, 1274, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2,
         2,
         2, 2, 2, 2, 2, 2, 2]
    )
