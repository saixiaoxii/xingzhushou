...
class QuestionClassifier():
    def __init__(self):
        ...
        # 特征词路径
        self.poem_path = os.path.join(cur_dir, 'poem.txt')
        self.dynasty_path = os.path.join(cur_dir, 'dynasty.txt')
        self.category_path = os.path.join(cur_dir, 'category.txt')
        self.author_path = os.path.join(cur_dir, 'author.txt')

        # 加载特征词
        self.poem_wds = [i.strip() for i in open(self.poem_path, encoding="utf-8", errors='ignore') if i.strip()]
        self.dynasty_wds = [i.strip() for i in open(self.dynasty_path, encoding="gbk",errors='ignore') if i.strip()]
        self.category_wds = [i.strip() for i in open(self.category_path, encoding="gbk",errors='ignore') if i.strip()]
        self.author_wds = [i.strip() for i in open(self.author_path, encoding="gbk",errors='ignore') if i.strip()]
